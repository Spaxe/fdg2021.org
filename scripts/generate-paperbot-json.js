...
function parseEntry (entry, dayOffset, overrideTime, overrideTrack) {
  ...
  if (entryTime !== 'TBA') {
    const time = dateFns.parse(entryTime, "HH:mm", new Date(2021, 7, 2 + dayOffset));
    let endTime;
    ...
    timeString = `${dateFns.format(time, "HH:mm")}-${dateFns.format(endTime, "HH:mm")}`;
